...
export default function ProfilePage() {
    ...
    const [hiddenPhone, setHiddenPhone] = useState(currentUser.user.phone)
    const [hiddenEmail, setHiddenEmail] = useState(currentUser.user.email)
    ...
    useEffect(() => {
        if (hiddenPhone) {
            let getNumberFront = hiddenPhone.slice(0, 3)
            let getNumberBack = hiddenPhone.slice(-2)
            let changeNumberMiddle = '*'.repeat(hiddenPhone.length - 5)
            let mergerPhone = getNumberFront + changeNumberMiddle + getNumberBack
            setHiddenPhone(mergerPhone)
        }
    }, [currentUser])
    // handler hidden email
    useEffect(() => {
        if (hiddenEmail) {
            let getEmailFront = hiddenEmail.slice(0, 3)
            let getEmailBack = hiddenEmail.slice(-2)
            let changeEmailMiddle = '*'.repeat(hiddenPhone.length - 5)
            let mergerEmail = getEmailFront + changeEmailMiddle + getEmailBack
            setHiddenEmail(mergerEmail)
        }
    }, [currentUser])
                ...
                <div className="profile-left my-10 lg:mr-5 col-span-1">
                    <div className="profile-left-card">
                        <Card
                            ...
                            <p className='card-name lg:my-3'>{currentUser.user.name}</p>
                        ...
                        >
                            ...
                            <div className='card-skills my-4'>
                                <div className='skills card-information-tittle items-center'>
                                    ...
                                    {isLengthSkill ? (
                                        ...
                                        <Button className='button-edit font-medium'>Add new</Button>
                                    )}

                                </div>
                                <div className='skills card-information-detail mb-5'>
                                    <span className='text-detail'>{currentUser.user.skill}</span>
                                </div>
                            </div>
                            ...
                            <div className='card-certifications my-4'>
                                ...
                                <div className='certification card-information-detail mb-5'>
                                    <span className='text-detail'>{currentUser.user.certification}</span>
                                </div>
                            </div>
